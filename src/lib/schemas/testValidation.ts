...
export const groupNameSchema = z.string().regex(DB_STRING_REGEX, { message: DB_STRING_MESSAGE }).min(GROUP_NAME_MIN, `Mark has to be at least ${GROUP_NAME_MIN} character long.`).max(GROUP_NAME_MAX, `Mark can be max ${GROUP_NAME_MAX} characters long.`)
