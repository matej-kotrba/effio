...
const includedInGroupsInputSchema = z
	.object({ isPublic: z.boolean(), subcategorySelect: includedInGroupsSchema })
	.optional();

export const protectedRouter = router({
	saveTest: loggedInProcedure
		.input(
			z.object({
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (
				...
				input.includedInGroups?.subcategorySelect?.some(
					(item) => item.id !== ''
				)
			) {
				throw new TRPCError({
					code: 'BAD_REQUEST',
					message: 'You cannot connect draft test to groups'
				});
			}
			...
			const result = await trpcCheckForRateLimit(
				'testCreation',
				ctx.userId,
				'creating tests'
			);
			...
			const isPublic = input.includedInGroups
				? input.includedInGroups.isPublic
				: true;
			const includedInGroups = input.includedInGroups
				? input.includedInGroups.subcategorySelect?.filter((item) => item.id)
				: [];
			...
			if (legitGroupsCount !== includedInGroups.length) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message:
						'You are not allowed to create a test in group you are not part of'
				});
			}
	...
	updateTest: loggedInProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			// Tags
			const tagsToDelete = test.tags.filter(
				(item) => !input.tagIds?.includes(item.tagId)
			);
			const tagsToCreate = input.tagIds?.filter(
				(item) => !test.tags.map((item) => item.tagId).includes(item)
			);
			...
			try {
				...
				// Groups
				const linkedGroupsToDelete = linkedGroups.filter(
					(item) =>
						!includedInGroups
							.map((item) => item.id)
							.includes(item.subcategoryId)
				);
				const linkedGroupsToCreate = includedInGroups.filter(
					(item) =>
						!linkedGroups.map((item) => item.subcategoryId).includes(item.id) &&
						item.id
				);
				...
				if (!testData) {
					throw new TRPCError({
						...
						message: 'Test version not created'
					});
				}
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				for (const question of test.testVersions[0].questions) {
					...
					const typeCheckedFunction =
						questionContentFunctions[
							question.type.slug as keyof QuestionTypeMap
						].onActionWithDB;
	...
	starTest: loggedInProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (test.ownerId === ctx.userId) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message: 'You are not allowed to star your own test'
				});
			}
	...
	createTestAttempt: loggedInProcedure
		.input(
			z.object({
				testId: z.string(),
		...
		.mutation(async ({ ctx, input }) => {
			...
			const result = await trpcCheckForRateLimit(
				'testAttemptCreation',
				ctx.userId,
				'attempting to take tests'
			);
			if (result) {
				throw result;
			}

			const subcategoryOnTest =
				await ctx.prisma.groupSubcategoryOnTests.findUnique({
					where: {
						testId_subcategoryId: {
			...
			if (subcategoryOnTest.test.ownerId === ctx.userId) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message: 'You cannot attempt your own test'
				});
			}

			const usersAttemptsCount =
				await ctx.prisma.userSubcategoryTestStartLog.count({
					where: {
						userId: ctx.userId,
						testId: input.testId,
						subcategoryId: input.subcategoryId
			...
			if (
				subcategoryOnTest.numberOfTries !== null &&
				usersAttemptsCount >= subcategoryOnTest.numberOfTries
			) {
				throw new TRPCError({
					code: 'FORBIDDEN',
	...
	getTestsOfUser: loggedInProcedure
		.input(
			z.object({
	...
	updateSubcategoryConnectionsToTest: loggedInProcedure
		.input(
			z.object({
				groupId: z.string(),
				subcategoryId: z.string(),
				testsToConnect: z.array(
					z.object({ id: z.string(), title: z.string() })
				),
				connectionsToDelete: z.array(
					z.object({
						id: z.number(),
						testId: z.string(),
						testTitle: z.string(),
		...
		.mutation(async ({ ctx, input }) => {
			const result = await trpcCheckForRateLimit(
				'channelConnectionToTests',
				ctx.userId,
				'connecting tests to channels'
			);
			if (result) {
				throw result;
			}
			...
			if (subcategory.group.ownerId !== ctx.userId) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message: 'You are not allowed to connect tests to this channel'
				});
			}
			...
			for (const message of messages) {
				pusher.trigger(
					`group-${input.groupId}-${input.subcategoryId}`,
					'new-message',
					message
				);
			}
	...
	toggleUserFollow: loggedInProcedure
		.input(
			z.object({
				userId: z.string()
			})
	...
	isUserFollowing: loggedInProcedure
		.input(
			z.object({
				userId: z.string()
			})
